...
import { generatePgTriggersDiffSql } from "../diff/pg-triggers-diff";

const program = new Command();
program
  .name("frg-data-diff pg-triggers")
  .description(
    "Compare PostgreSQL triggers and functions and write a SQL diff script.",
  )
...
async function main() {
  ...
  const cliArgs: Partial<ResolvedGeneratorOptions> = {
  ...
  ) as Partial<ResolvedGeneratorOptions>;
  ...
  try {
    ...
    const sql = await generatePgTriggersDiffSql(
      sourcePool,
      destPool,
      diffOptions,
    );
